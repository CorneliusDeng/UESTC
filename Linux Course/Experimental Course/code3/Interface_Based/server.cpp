...
class TCP_Server
{
    private:
        ...
        TCP_Server_Observer* m_pObserver; // 服务端历程


    public:
        TCP_Server(TCP_Server_Observer *pObserver, int nServerPort, int nLengthOfQueueOfListen = 100, const char *strBoundIP = NULL){
...
class My_TCP_Server : public TCP_Server_Observer
{
    public:
        My_TCP_Server(){
        }
...
int main(){
        My_TCP_Server myserver;
        TCP_Server tcpserver(&myserver, 3845);
        tcpserver.Run();
        return 0;
}