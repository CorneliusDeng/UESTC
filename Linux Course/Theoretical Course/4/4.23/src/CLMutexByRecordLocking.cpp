...
CLMutexByRecordLocking::CLMutexByRecordLocking(const char *pstrFileName)
{
	...
	if(m_Fd == -1)
	{
		CLLogger::WriteLogMsg("In CLMutexByRecordLocking::CLMutexByRecordLocking(), open error", errno);
...
{
	if(close(m_Fd) == -1)
		CLLogger::WriteLogMsg("CLMutexByRecordLocking::~CLMutexByRecordLocking(), close error", errno);
}
