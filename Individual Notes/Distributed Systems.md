...
逻辑时钟：众多应用只要求所有节点具有相同时间基准，该时间不一定与物理时间相同，但所有的进程需要在事件的发生顺序上一致。
...
- 串行等价
  ...
  - 两个事务串行等价的**充要条件**是：**两个事务中所有的冲突操作都按相同的次序在它们访问的对象上执行**
...
- Coda文件系统
  ...
  - **复制策略**
    ...
    - 例如：CVV=（2，2，1）
      ...
      - 文件在服务器3上收到1个更新
    - **冲突检测**
      - 若一个站点的CVV大于或等于所有其它站点相应的CVV，则不存在冲突。例如[2,2,1]，[1,1,1]→自动更新
      - 若对于两个CVV而言，v1≥v2与v2≥v1均不成立，则存在一个冲突。例如[2,2,1]，[1,1,3] →手工干预
    - **文件关闭**
...
- **GFS设计思想**
  ...
  - **Master服务器在不同的数据文件里保存元数据。**数据以64MB为单位存储在文件系统中。Client与Master服务器通讯在文件上做元数据操作并且找到包含用户需要的数据
...
- 假定有一个分布式WEB缓存系统，那么其**数据缓存的算法**可以有两种
